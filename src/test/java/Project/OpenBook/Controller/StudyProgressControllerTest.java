...
public class StudyProgressControllerTest {
    ...
    public class addChapterProgressWrongCount{
        @BeforeAll
        public void init(){
        ...
        public void addChapterProgressSuccessFirst() {
            ...
            Optional<ChapterProgress> chapterProgressOptional = chapterProgressRepository.queryChapterProgress(customer1.getId(), ch1.getNumber());
    ...
    @TestInstance(PER_CLASS)
    public class addChapterProgressProgressUpdate{
        @BeforeAll
        public void init(){
        ...
        @Test
        public void addChapterProgressSuccessFirst() {
            ProgressDto dto = new ProgressDto(ch1.getNumber(), ProgressConst.TIME_FLOW_QUESTION);
            ...
            assertThat(response.getStatusCode()).isEqualTo(HttpStatus.CREATED);
            Optional<ChapterProgress> chapterProgressOptional = chapterProgressRepository.queryChapterProgress(customer1.getId(), ch1.getNumber());
            assertThat(chapterProgressOptional.get().getProgress()).isEqualTo(ProgressConst.TIME_FLOW_QUESTION);
        }
        @DisplayName("단원 학습 정보 입력 실패 - DTO Validation")
        @Test
        public void addChapterProgressFailWrongDto() {
        ...
        @Test
        public void addChapterProgressFailNotFoundChapterNum(){
            ProgressDto dto = new ProgressDto(-1, ProgressConst.GET_TOPIC_BY_KEYWORD);
            ResponseEntity<List<ErrorMsgDto>> response = restTemplate
                    .withBasicAuth("customer1", "customer1")
                    .exchange(URL, HttpMethod.POST, new HttpEntity<>(dto), new ParameterizedTypeReference<List<ErrorMsgDto>>() {
                    });
