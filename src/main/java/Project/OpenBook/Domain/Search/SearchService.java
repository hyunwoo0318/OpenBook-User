...
public class SearchService {
    ...
    public SearchResultDto searchByInput(String input) {
        ...
        List<TopicSearch> topicSearchList = topicSearchRepository.queryTopicSearchNameByInput(
            input);
        List<KeywordSearch> keywordNameSearchList = keywordSearchRepository.queryKeywordSearchNameByInput(
            input);
        List<ChapterSearch> chapterSearchList = chapterSearchRepository.queryChapterSearchNameByInput(
            input);
        ...
        List<TopicSearchResultDto> topicList = topicSearchList.stream()
            .map(ts -> new TopicSearchResultDto(ts.getChapterNumber(), ts.getChapterTitle(),
                ts.getTitle()))
        ...
        List<KeywordSearchResultDto> keywordNameList = keywordNameSearchList.stream()
            .map(ks -> new KeywordSearchResultDto(ks.getChapterNumber(), ks.getChapterTitle(),
                ks.getTopicTitle(),
