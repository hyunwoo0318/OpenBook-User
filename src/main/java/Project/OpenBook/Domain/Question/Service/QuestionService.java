...
public class QuestionService {
  ...
  public QuestionService(
      ...
      KeywordPrimaryDateRepository keywordPrimaryDateRepository,
      TopicPrimaryDateRepository topicPrimaryDateRepository,
      TopicLearningRecordRepository topicLearningRecordRepository,
    ...
    this.type1 =
        new GetKeywordByTopicQuestion(
            this.topicRepository, this.keywordRepository, this.weightedRandomService);
    this.type2 =
        new GetTopicByKeywordQuestion(
            this.topicRepository, this.keywordRepository, this.weightedRandomService);
  }
   ...
   */
  private static void topicPrimaryDateToQuestionDto(
      PrimaryDateQueryResult result, List<TimeFlowQuestionDto> timeFlowQuestionDtoList) {
    for (TopicPrimaryDate tp : result.topicPrimaryDateList) {
      ...
      List<String> keywordList =
          tp.getTopic().getKeywordList().stream()
   ...
   * @param timeFlowQuestionDtoList
   */
  private static void keywordPrimaryDateToQuestionDto(
      PrimaryDateQueryResult result, List<TimeFlowQuestionDto> timeFlowQuestionDtoList) {
    for (KeywordPrimaryDate kp : result.keywordPrimaryDateList) {
      ...
      if (!keywordComment.isBlank()) {
        List<String> splitCommentList =
            Arrays.stream(keywordComment.split("[.]")).collect(Collectors.toList());
  ...
  /**
   * timelineId에 맞게 primaryDate를 쿼리하는 로직 timelineId == RANDOM_INDEX -> 전체 timeline중 랜덤하게 1개를 선정해 해당
   * timeline에서 primaryDate 조회 timelineId == TOTAL_INDEX -> 전체 timeline의 모든 primaryDate조회
   *
   * @param timelineId
   * @return
   */
  private PrimaryDateQueryResult getPrimaryDateQueryResult(Long timelineId) {
    ...
    if (timelineId == TOTAL_INDEX) {
      topicPrimaryDateList =
          topicPrimaryDateRepository.queryTopicPrimaryDateInTimeline(TOTAL_INDEX, 0, 0);
      keywordPrimaryDateList =
          keywordPrimaryDateRepository.queryKeywordPrimaryDateInTimeline(TOTAL_INDEX, 0, 0);
      ...
      Timeline timeline =
          timelineRepository
              .queryTimelineWithEra(timelineId)
              .orElseThrow(
                  () -> {
                    throw new CustomException(TIMELINE_NOT_FOUND);
                  });
      ...
      topicPrimaryDateList =
          topicPrimaryDateRepository.queryTopicPrimaryDateInTimeline(eraId, startDate, endDate);
      keywordPrimaryDateList =
          keywordPrimaryDateRepository.queryKeywordPrimaryDateInTimeline(eraId, startDate, endDate);
    }
  ...
  public List<QuestionDto> queryRandomQuestion(Customer customer, Long questionCategoryId) {
    ...
    if (questionCategoryId == -1L) {
      questionCategory =
          questionCategoryLearningRecordRepository
              .queryQuestionCategoryLowScore(customer)
              .getQuestionCategory();
      questionCategoryId = questionCategory.getId();
    } else {
      questionCategory =
          questionCategoryRepository
              .queryQuestionCategoriesWithTopicList(questionCategoryId)
              .orElseThrow(
                  () -> {
    ...
    Map<Keyword, KeywordLearningRecord> keywordRecordMap =
        keywordLearningRecordRepository
            .queryKeywordLearningRecordsInQuestionCategory(customer, questionCategoryId)
            .stream()
    ...
    List<QuestionDto> type1QuestionList =
        type1.getQuestion(
            keywordRecordMap, totalKeywordList, QuestionConst.GET_TOPIC_QUESTION_COUNT);
    List<QuestionDto> type2QuestionList =
        type2.getQuestion(
            keywordRecordMap, totalKeywordList, QuestionConst.GET_KEYWORD_QUESTION_COUNT);
